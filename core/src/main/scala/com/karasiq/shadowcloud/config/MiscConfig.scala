...
private[shadowcloud] final case class MiscConfig(rootConfig: Config, lifecycleHooks: Seq[Class[LifecycleHook]], syncOnExit: Boolean)
...
private[shadowcloud] object MiscConfig extends WrappedConfigFactory[MiscConfig] with ConfigImplicits {
  override def apply(config: Config): MiscConfig = {
    ...
    MiscConfig(config, hooks, config.getBoolean("sync-on-exit"))
  }
}
