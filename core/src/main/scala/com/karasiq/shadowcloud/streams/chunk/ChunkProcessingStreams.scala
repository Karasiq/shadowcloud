...
final class ChunkProcessingStreams(cryptoModules: CryptoModuleRegistry, chunks: ChunksConfig, crypto: CryptoConfig, parallelism: ParallelismConfig)(
    implicit ec: ExecutionContext
) {
  type ChunkFlow = Flow[Chunk, Chunk, NotUsed]

  def defaultChunkSize: Int = chunks.chunkSize

  def split(chunkSize: Int = defaultChunkSize): Flow[ByteString, Chunk, NotUsed] = {
    ChunkSplitter(chunkSize)
  }

  def generateKeys(method: EncryptionMethod = crypto.encryption.chunks, maxKeyReuse: Int = crypto.encryption.maxKeyReuse): ChunkFlow = {
  ...
  def encrypt: ChunkFlow =
    parallelFlow(parallelism.encryption) { chunk ⇒
      chunk.data match {
  ...
  def createHashes(plainMethod: HashingMethod = crypto.hashing.chunks, encMethod: HashingMethod = crypto.hashing.chunksEncrypted): ChunkFlow = {
  ...
  def beforeWrite(
      encryption: EncryptionMethod = crypto.encryption.chunks,
  ...
  def index(
      plainHashing: HashingMethod = crypto.hashing.files,
      encHashing: HashingMethod = crypto.hashing.filesEncrypted
  ): Sink[Chunk, Future[FileIndexer.Result]] = {
