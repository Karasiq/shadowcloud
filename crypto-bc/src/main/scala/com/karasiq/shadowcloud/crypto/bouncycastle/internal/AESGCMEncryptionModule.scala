...
private[bouncycastle] final class AESGCMEncryptionModule(bits: Int = 256) extends StreamEncryptionModule {
  private[this] val secureRandom = SecureRandom.getInstanceStrong
  private[this] val keyGenerator = KeyGenerator.getInstance("AES", BCUtils.provider)
  keyGenerator.init(bits, secureRandom)
  private[this] val aes = new GCMBlockCipher(new AESEngine) // TODO: AESFastEngine
