...
object CryptoException {
  final case class ChecksumError(expected: ByteString, actual: ByteString)
      extends CryptoException("Checksum not match: " + HexString.encode(expected) + " / " + HexString.encode(actual))
