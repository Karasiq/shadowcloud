...
object StorageException {
  final case class NotFound(path: Path, cause: Throwable = null)
    extends StorageException("Not found: " + path, cause) with SCException.NotFound

  final case class AlreadyExists(path: Path, cause: Throwable = null)
    extends StorageException("Already exists: " + path, cause) with SCException.AlreadyExists
