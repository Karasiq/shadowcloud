...
trait DiffMergeDecider[T] {
  def apply(leftCreates: Set[T], leftDeletes: Set[T], rightCreates: Set[T], rightDeletes: Set[T], splitDecider: SplitDecider[T]): (Set[T], Set[T])
}

//noinspection ConvertExpressionToSAM
object DiffMergeDecider {
  def idempotent[T]: DiffMergeDecider[T] = new DiffMergeDecider[T] {
    def apply(leftCreates: Set[T], leftDeletes: Set[T], rightCreates: Set[T], rightDeletes: Set[T], splitDecider: SplitDecider[T]) = {
