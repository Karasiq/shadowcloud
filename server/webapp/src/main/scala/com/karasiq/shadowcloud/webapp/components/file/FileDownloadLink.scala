...
object FileDownloadLink {
  def apply(file: File, useId: Boolean = false)(title: Modifier*)(implicit context: AppContext, folderContext: FolderContext): FileDownloadLink = {
    new FileDownloadLink(file, useId)(title)
  }
}

final class FileDownloadLink(file: File, useId: Boolean)(title: Modifier*)(implicit context: AppContext, folderContext: FolderContext)
