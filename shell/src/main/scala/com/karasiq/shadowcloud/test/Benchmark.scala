...
private object Benchmark extends App {
  ...
  private[this] def runWriteBenchmark(
      encryption: EncryptionMethod = sc.config.crypto.encryption.chunks,
      hashing: HashingMethod = sc.config.crypto.hashing.chunks,
      fileHashing: HashingMethod = sc.config.crypto.hashing.files
  ): Unit = {
    val modifier   = 1
  ...
  private[this] def runReadBenchmark(): Unit = {
    ...
    val future = Source
      .future(sc.ops.region.getFolderIndex("testRegion"))
    ...
    val speed   = 1.second / perMb
    println(
      f"Read benchmark completed, ${MemorySize(bytes)}, ${elapsed.toSeconds} seconds elapsed, ${perMb.toMillis} ms per megabyte, $speed%.2f MB/sec"
    )
  }
