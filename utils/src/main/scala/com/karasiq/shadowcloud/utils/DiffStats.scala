...
case class DiffStats(
  ...
  require(
    diffs >= 0 && files >= 0 && folders >= 0 && chunks >= 0 &&
  ...
  def +(stat: DiffStats): DiffStats = {
    DiffStats(
      ...
      chunks + stat.chunks,
      fileDeletes + stat.fileDeletes,
      folderDeletes + stat.folderDeletes,
      chunkDeletes + stat.chunkDeletes,
      createdBytes + stat.createdBytes,
      deletedBytes + stat.deletedBytes
    )
  }
